...
class SolveDiffusion2D:
    ...
    def initialize_domain(self, w=10., h=10., dx=0.1, dy=0.1):
        ...
        assert type(dy) == float, "dy is not float"
        self.w = w
        self.h = h
        self.dx = dx
        self.dy = dy
    ...
    def initialize_physical_parameters(self, d=4., T_cold=300., T_hot=700.):
        assert type(d) == float, "d is not float"
        assert type(T_cold) == float, "T_cold is not float"
        assert type(T_hot) == float, "T_hot is not float"
        self.D = d
        self.T_cold = T_cold
        self.T_hot = T_hot
